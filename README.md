...
## Bolt.New and Coolify Integration

To configure Bolt.New for Coolify and ensure compatibility, follow these additional steps:

### Configuration Tab:

1. **Domains**: 
   - You can let Coolify autogenerate domain names, but your VPS provider must support wildcard subdomain names for DNS records. For example, Hostinger does not support wildcard subdomains, but you can manually add A or CNAME records for production and development.

2. **Domains for App Prod**: 
   - Use autogenerated or custom A/CNAME records on the VPS for custom domain names (e.g., `https://pro.bolt.domainname.com`).

3. **Domains for App Dev**:
   - Use autogenerated or custom A/CNAME records on the VPS for custom domain names (e.g., `https://dev.bolt.domainname.com`).

4. **Custom Build Command**:
   - For the development environment: `docker build . --target bolt-ai-development`
   - For the production environment: `docker build . --target bolt-ai-production`

5. **Custom Start Command**:
   - After setting up the build command, click on “Reload Compose File” to regenerate any changes in the `docker-compose.yaml` file.

6. **Pre/Post Deployment Commands**:
   - Specify pre-deployment or post-deployment commands for your container if needed.

### Environment Variables:

1. Update the API keys so Coolify can create and update the `.env` script when creating the container.
2. Set the following fields:
   - `COOLIFY_FQDN` (e.g., `pro.bolt.domainname.com`)
   - `COOLIFY_URL` (e.g., `https://pro.bolt.domainname.com`)

This setup ensures Bolt.New operates seamlessly within the Coolify environment, catering to both development and production needs.
